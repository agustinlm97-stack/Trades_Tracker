...
# streamlit run futures_ticker2.py
...
.top-title {
    font-size:0.58rem; color:#2a4a5a; letter-spacing:4px; margin-bottom:5px;
}
...
/* POSITION CARDS */
.pos-card { background:#0a1018; border:1px solid #0f2a3a; border-left:3px solid #2a4a5a; padding:16px 18px; height:100%; }
...
.closed-title { font-size:0.58rem; color:#2a4a5a; letter-spacing:4px; margin-bottom:14px; }
.closed-row { display:flex; gap:10px; align-items:center; margin-bottom:8px; }
.closed-total-pos { font-size:1.6rem; font-weight:700; color:#00ff88; }
.closed-total-neg { font-size:1.6rem; font-weight:700; color:#ff3366; }
.closed-total-neu { font-size:1.6rem; font-weight:700; color:#ffd700; }
.grand-bar {
    background:#0a1018;
...
def api_price(symbol):
    try:
        r = requests.get(f"{FAPI}/ticker/price", params={"symbol": symbol}, timeout=5)
        r.raise_for_status()
...
for sym in tracked:
    p = api_price(sym)
    if p:   st.session_state.live[sym] = p
...
# Render positions in pairs (2 columns)
...
<div class="pos-card {status}">
  ...
  <div class="detail-row">
    ...
    <div class="d-item"><label>NOCIONAL</label><div class="d-val">${notional:,.2f}</div></div>
                ...
                s_cls = "b-long" if side=="Long" else "b-short"
                warn  = " - sin precio Binance" if pd.get("current") is None else ""
                st.markdown(f"""
...
with st.expander("+ AGREGAR TRADE CERRADO", expanded=len(st.session_state.closed_trades) == 0):
    ...
    with cc1:
        c_label = st.text_input("DESCRIPCION (opcional)", placeholder="ej: BTC Long 10x", key="c_label")
    with cc2:
        c_pnl   = st.number_input("P&L REALIZADO (USDT)", value=0.0, format="%.2f", step=0.01, key="c_pnl")
    with cc3:
        c_date  = st.text_input("FECHA (opcional)", placeholder="ej: 15/03/2026", key="c_date")
    with cc4:
        st.markdown("<div style='height:26px'></div>", unsafe_allow_html=True)
...
with cs:
    if errors:
        st.markdown(f"<span class='status-err'>sin precio: {', '.join(errors)}</span>", unsafe_allow_html=True)
    else:
        st.markdown(f"<span class='status-ok'>LIVE - {st.session_state.last_update}</span>", unsafe_allow_html=True)
with cr:
    ref = st.selectbox("Refresh", ["5s","10s","30s"], label_visibility="collapsed")

time.sleep(int(ref[:-1]))
st.rerun()
